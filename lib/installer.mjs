...
const ROUTES = [
  ...
  { skill: "teach", when: "learning a topic" },
  { skill: "write-skill", when: "authoring a skill" },
];
...
export function install({ skillsSource, runtimes, mode = "copy", hookSource }) {
  ...
  // Only copy mode writes libs into a user's skills dir, so the clobber risk
  // (and thus the warning) only applies there. Link mode copies nothing.
  if (mode === "copy") {
